...
# Check whether relevant dependencies exist
def check_tools():
    tools = {
        "java": "java -version",
        "perl": "perl -v",
        "bedGraphToBigWig": "bedGraphToBigWig", # Will fail if not in PATH
    ...
    for tool, command in tools.items():
        try:
            # For bedGraphToBigWig, just check existence, as running it without args might error
...
rule finaleme_train_model:
    input:
        ...
        # features=os.path.join(OUT_DIR, "{sample}", "{sample}.CpgMultiMetricsStats.details.bed.gz") # Alternative if not chaining
    output:
        model=os.path.join(OUT_DIR, "{sample}", "{sample}.finaleme.model"),
        # The training step also produces a prediction file, let's name it distinctly
...
rule finaleme_decode:
    input:
        ...
        features=rules.finaleme_extract_features.output.features # Use original features
        # model=os.path.join(OUT_DIR, "{sample}", "{sample}.finaleme.model"),
        # features=os.path.join(OUT_DIR, "{sample}", "{sample}.CpgMultiMetricsStats.details.bed.gz")
    output:
...
rule finaleme_convert_to_bigwig:
    ...
    output:
        # Assuming the perl script outputs {prefix}.cov.b37.bw and {prefix}.methy_count.b37.bw
        # The prefix is the first argument to the perl script.
        # The output name in the example is `test.cov.b37.bw`.
        # We use `{sample}.finaleme` as the base for our output prefix.
    ...
    params:
        ...
        # The prefix for the output files generated by the perl script
    ...
    shell:
        ...
        """
        # After running, the script should have created:
        # {params.output_prefix}.cov.b37.bw -> matches {output.cov_bw}
        # {params.output_prefix}.methy_count.b37.bw -> matches {output.methy_count_bw} (ASSUMPTION!)

# --- Tissue of Origin Analysis (Step 5) ---

# Step 5a: Generate the command string file for AlignMultiWigInsideBed
rule too_generate_alignmultiwig_cmd_file:
    input:
        # Depends on all methy_count.bw and cov.bw files being created
    ...
    run:
        if config.get("too_enabled"):
            ...
            with open(output.cmd_file, "w") as f:
                cmd_parts = []
                # Ensure SAMPLES are sorted for consistent command generation if order matters
                sorted_samples = sorted(SAMPLES)
                for sample_id in sorted_samples:
                    # Construct paths based on the output rule of finaleme_convert_to_bigwig
                    ...
                    # Check if files actually exist (Snakemake should ensure this if dependencies are right)
                    # if not os.path.exists(methy_count_file):
                    #     raise FileNotFoundError(f"Missing expected methy_count file: {methy_count_file}")
                    # if not os.path.exists(cov_file):
                    #     raise FileNotFoundError(f"Missing expected cov file: {cov_file}")
...
# Step 5b: Generate names order file
    ...
    run:
        if config.get("too_enabled"):
            ...
            with open(output.names_file, "w") as f:
                # Ensure SAMPLES are sorted for consistent order
                sorted_samples = sorted(SAMPLES)
                for i, sample_id in enumerate(sorted_samples):
                    # The original perl script extracts a name. Here we just use the sample_id.
                    # Path to one of the BW files to extract the base name if needed,
                    # but since we have SAMPLES, we can use that.
                    # Example: sample_name = sample_id (adjust if a more complex name is needed)
                    f.write(f"{i+1}\t{sample_id}\n")
        else:
            shell("mkdir -p $(dirname {output.names_file}) && touch {output.names_file}")

# Step 5c: Run AlignMultiWigInsideBed
rule too_run_alignmultiwig:
    input:
        ...
        # Explicitly list dependencies to ensure they are built before this rule runs
        methy_counts=expand(rules.finaleme_convert_to_bigwig.output.methy_count_bw, sample=SAMPLES),
        covs=expand(rules.finaleme_convert_to_bigwig.output.cov_bw, sample=SAMPLES)
    output:
